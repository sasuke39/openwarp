...
- 每次请求重新读取策略；新版 MCP Access 开关和服务器授权对后续调用即时生效。
- 总开关关闭时拒绝查询、输入、取消等所有新调用，已接收的命令继续执行。
- `server_permissions` 分别管理命令、上传、下载；显式项覆盖旧 `server_ids`。
- 本地下载根目录仍受启动时策略约束，手动扩大 `transfer_roots` 后需重启。
- 缺失策略的错误包含策略路径。
