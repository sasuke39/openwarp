...
`workspace.shell` accepts `command`, `workdir`, and `executionMode`. The mode
is required and is either `foreground` or `background`; omitted/unknown modes
fail closed. Long or uncertain commands use `background`. If a foreground
command outlives its observation window, the client keeps the same managed
process and requires read/wait/cancel or explicit detach before unrelated work.
