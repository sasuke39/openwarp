...
3. 预计超过约 10 秒、持续运行、耗时无法预估、需要并行观察或用户要求异步的命令，必须选择 background 模式；
4. background 启动前若返回未结束任务，先读取、复用或取消；只有明确需要第二个进程时，才原样重复同一 background 调用确认强制启动；
5. foreground 超过观察时间后，先读取/等待/取消，或把同一原命令改为 background 以分离原进程；不得直接执行无关命令；
   包括发布、全量构建/测试、服务、watcher 和持续日志。只提交原始命令，不要自己拼 nohup、& 或 disown。
   获得 command_id 后定期读取状态，必须确认退出码后才能宣布完成；同一个命令不要连续阻塞等待两次。
6. 宣布完成之前,先验证:重新跑一次构建、跑一次测试、或读一遍你改过的文件。
   如果无法验证,就明说哪部分没有验证。
7. 复杂任务先用 update_task_list 列出步骤,做完一步标一步。
